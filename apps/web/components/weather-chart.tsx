...
const CITY_COLORS = ["#134e4a", "#c97a40", "#3b82f6", "#7c3aed", "#ef4444"];
  ...
  return (
    <div className="glass-panel terminal-panel px-6 py-6 md:px-7">
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div>
          <div className="text-xs uppercase tracking-[0.24em] text-ink/45">Realtime</div>
          <h2 className="mt-2 font-display text-4xl leading-none text-ink">Temperature rhythm</h2>
        </div>
        <div className="text-sm text-ink/55">Last several readings per subscribed city, refreshed as inserts arrive</div>
      </div>
      <div className="mt-6 h-64 md:h-72">
        {cityNames.length === 0 ? (
          <div className="flex h-full items-center justify-center rounded-[24px] border border-dashed border-black/10 bg-white/35 px-6 text-center text-sm leading-7 text-ink/55">
          ...
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={chartData}>
              ...
              <XAxis dataKey="label" tickLine={false} axisLine={false} stroke="rgba(22, 33, 43, 0.45)" />
